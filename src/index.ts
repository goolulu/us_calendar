...
export async function buildEvents(now: Date): Promise<BuildResult> {
  ...
  return { events: inWindow(events, now), sources, generatedAt: now.toISOString() };
}
...
export default {
  async fetch(request: Request, _env: unknown, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      const result = await buildEvents(new Date());
      const ok = result.events.length > 0 && result.sources.some((source) => source.state === "ok");
      return Response.json({ ok, generatedAt: result.generatedAt, events: result.events.length, sources: result.sources }, { status: ok ? 200 : 503, headers: { "cache-control": "no-store" } });
    }
    ...
    const response = await calendarResponse(new Date());
